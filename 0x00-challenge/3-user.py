...
class User():
    ...
    def __init__(self):
        ...
        self.__password = None  # Fix: Initialize the __password attribute here
    ...
    def password(self, pwd):
        ...
        """
        if pwd is None or not isinstance(pwd, str):  # Fix: Use isinstance to check for string
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # Fix: Correctly assign to __password
    ...
    def is_valid_password(self, pwd):
        ...
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # Fix: Correctly compare with __password
